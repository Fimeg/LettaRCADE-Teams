...
import { createConversationTarget, getConversationTarget, getRootConversationId, loadTeammate, saveTeammate, teammateExists, updateConversationTarget, updateStatus, listTeammates, getApiKey, } from "./store.js";
 ...
 */
async function withTeammateLock(name, fn) {
...
export async function forkTeammate(rootName, forkName) {
    ...
    return withTeammateLock(rootName, async () => {
        ...
        try {
            const session = __addDisposableResource(env_1, createSession(state.agentId, {
...
async function withRetry(fn, options = {}) {
    const { maxAttempts = 3, baseDelayMs = 1000, maxDelayMs = 30000, shouldRetry = isRetryableError, } = options;
    let lastError;
...
export function checkApiKey() {
    ...
    if (!apiKey) {
        throw new Error("No API key found. Run 'letta-teams auth' to configure, or set LETTA_API_KEY environment variable.");
    }
 ...
 */
function validateMemfsStartup(mode) {
    if (mode !== undefined && !MEMFS_STARTUP_VALUES.includes(mode)) {
        throw new Error(`Invalid memfs-startup mode '${mode}'. Must be one of: ${MEMFS_STARTUP_VALUES.join(", ")}`);
    }
}
/**
...
function getDefaultModel() {
    if (process.env.LETTA_MODEL)
        return process.env.LETTA_MODEL;
    return isLettaCloud() ? "auto" : undefined;
}
function buildTeammateMemoryBlocks(name, role) {
    const identityBlock = {
        label: "identity",
 ...
 */
export async function spawnTeammate(name, role, options = {}) {
    ...
    if (teammateExists(name)) {
        throw new Error(`Teammate '${name}' already exists. Use --force to overwrite.`);
    }
    let agentId;
    try {
        ...
        agentId = await withRetry(() => createAgent({
            model,
        ...
        let conversationId;
        {
            ...
            try {
                const session = __addDisposableResource(env_2, createSession(agentId, {
                    permissionMode: "bypassPermissions",
                    disallowedTools: ["AskUserQuestion", "EnterPlanMode", "ExitPlanMode"],
                    memfs: memfsEnabled,
        ...
        if (!conversationId) {
            throw new Error("Failed to initialize teammate session: no conversation ID received from Letta API");
        }
 ...
 */
export async function initializeTeammateMemory(name, message, options = {}) {
    const { onEvent, onStreamEvent, maxDurationMs = DEFAULT_INIT_MAX_DURATION_MS, maxIdleMs = DEFAULT_INIT_MAX_IDLE_MS, } = options;
    ...
    return withTeammateLock(name, async () => {
        ...
        try {
            ...
            try {
                const session = __addDisposableResource(env_3, createSession(agentId, {
                    permissionMode: "bypassPermissions",
                    disallowedTools: ["AskUserQuestion", "EnterPlanMode", "ExitPlanMode"],
                    memfs: state.memfsEnabled,
                    memfsStartup: state.memfsStartup,
 ...
 */
export async function messageTeammate(name, message, options = {}) {
    ...
    return withTeammateLock(rootName, async () => {
        ...
        try {
            ...
            try {
                ...
                const session = __addDisposableResource(env_4, resumeSession(conversationId, {
                    permissionMode: "bypassPermissions",
                    disallowedTools: ["AskUserQuestion", "EnterPlanMode", "ExitPlanMode"],
                    memfs: state.memfsEnabled,
                    memfsStartup: state.memfsStartup,
 ...
 */
export async function broadcastMessage(message, options = {}) {
    ...
    const tasks = teammates.map((teammate) => limit(async () => {
        try {
 ...
 */
export async function dispatchMessages(messages, options = {}) {
    ...
    const limit = pLimit(concurrency);
    const results = new Map();
    const tasks = Array.from(messages.entries()).map(([name, message]) => limit(async () => {
        try {
