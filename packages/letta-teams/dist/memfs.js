...
let gitExec = execFileSync;
export function setMemfsGitExecutor(executor) {
    gitExec = executor;
}
export function resetMemfsGitExecutor() {
    gitExec = execFileSync;
}
export function getMemoryFilesystemRoot(agentId, homeDir = homedir()) {
    return join(homeDir, MEMORY_FS_ROOT, MEMORY_FS_AGENTS_DIR, agentId, MEMORY_FS_MEMORY_DIR);
}
export function getOwnedMemfsFiles() {
    return [...OWNED_MEMFS_FILES];
}
export function getMemorySystemDir(agentId, homeDir = homedir()) {
    return join(getMemoryFilesystemRoot(agentId, homeDir), MEMORY_SYSTEM_DIR);
}
export function ensureMemoryFilesystemDirs(agentId, homeDir = homedir()) {
...
function writeMemoryFile(agentId, relativePath, content) {
    const fullPath = join(getMemoryFilesystemRoot(agentId), relativePath);
    mkdirSync(dirname(fullPath), { recursive: true });
    writeFileSync(fullPath, content, "utf-8");
}
function renderFrontmatter(description, limit = 1200) {
    return `---
...
export function scaffoldTeammateMemfs(state, options = {}) {
    if (!state.memfsEnabled)
        return;
    ensureMemoryFilesystemDirs(state.agentId);
    writeMemoryFile(state.agentId, "system/teammate/identity.md", `${renderFrontmatter("Stable identity for this spawned teammate.")}
...
`);
    ...
    writeMemoryFile(state.agentId, "system/teammate/role.md", `${renderFrontmatter("Durable role definition and scope boundaries for this teammate.")}
...
`);
    writeMemoryFile(state.agentId, "system/teammate/contracts.md", `${renderFrontmatter("Operational teammate contracts for TODO, STATUS, and coordination.")}
...
\`\`\`
`);
    writeMemoryFile(state.agentId, "system/teammate/playbooks.md", `${renderFrontmatter("Reusable execution playbooks for teammate workflows.")}
...
`);
    writeMemoryFile(state.agentId, "system/teammate/quality-bar.md", `${renderFrontmatter("Durable engineering quality standards for this teammate.")}
...
`);
    writeMemoryFile(state.agentId, "system/project/context.md", `${renderFrontmatter("Project-level context captured at teammate spawn time.")}
...
`);
    writeMemoryFile(state.agentId, "system/init/status.md", `${renderFrontmatter("Initialization lifecycle status for this teammate.")}
...
export function isMemfsGitRepo(agentId, homeDir = homedir()) {
    return existsSync(join(getMemoryFilesystemRoot(agentId, homeDir), ".git"));
}
export function syncOwnedMemfsFiles(state, reason) {
    if (!state.memfsEnabled) {
        return { synced: false, committed: false };
    }
    if (!isMemfsGitRepo(state.agentId)) {
        throw new Error(`Memfs repo is not initialized for ${state.name} at ${getMemoryFilesystemRoot(state.agentId)}`);
    }
...
export function updateTeammateInitScaffold(state) {
    if (!state.memfsEnabled)
        return;
    ensureMemoryFilesystemDirs(state.agentId);
