...
import { getDaemonPidPath, getDaemonPort } from "./daemon.js";
 ...
 */
export function startDaemonInBackground() {
    ...
    const child = child_process.spawn(process.execPath, [daemonPath, "daemon", "--internal"], {
        detached: true,
 ...
 */
export async function sendToDaemon(msg, options) {
 ...
 */
export async function dispatchTask(targetName, message, options) {
    ...
    if (response.type !== "accepted") {
        throw new Error(response.type === "error" ? response.message : "Unexpected response from daemon");
    }
 ...
 */
export async function getTaskStatus(taskId, projectDir) {
    const response = await sendToDaemon({
        type: "status",
        taskId,
 ...
 */
export async function listTasks(projectDir) {
    const response = await sendToDaemon({
        type: "list",
        projectDir: projectDir ?? process.cwd(),
    });
 ...
 */
export async function dispatchAndWait(targetName, message, options) {
 ...
 */
export async function spawnTeammateViaDaemon(name, role, options) {
    const response = await sendToDaemon({
        type: "spawn",
        name,
        role,
...
export async function startCouncilViaDaemon(prompt, options) {
    const response = await sendToDaemon({
        type: 'council_start',
        prompt,
...
export async function killTeammateViaDaemon(name, options) {
    const response = await sendToDaemon({
        type: "kill",
        name,
        projectDir: options?.projectDir ?? process.cwd(),
...
export async function forkTeammateViaDaemon(rootName, forkName, options) {
    const response = await sendToDaemon({
        type: 'fork',
        rootName,
        forkName,
...
export async function reinitTeammateViaDaemon(rootName, options) {
    const response = await sendToDaemon({
        type: "reinit",
        rootName,
