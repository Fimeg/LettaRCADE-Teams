...
import { forkTeammate, messageTeammate, spawnTeammate, checkApiKey, initializeTeammateMemory, } from "./agent.js";
import { createTask, updateTask, getTask, listRecentTasks, listTasks, loadTasks, getGlobalAuthDir, ensureGlobalAuthDir, setProjectDir, getConversationTarget, loadTeammate, updateTeammate, updateStatus, createConversationTarget, updateConversationTarget, getRootConversationId, getMemoryTarget, findStaleRunningInitTasks, } from "./store.js";
import { buildInitPrompt, buildReinitPrompt, parseInitResult } from "./init.js";
import { parseTargetName } from './targets.js';
import { scaffoldTeammateMemfs, updateTeammateInitScaffold, syncOwnedMemfsFiles, } from "./memfs.js";
import { generateCouncilSessionId, runCouncilSession } from './council/orchestrator.js';
async function syncTeammateMemfs(teammateName, reason) {
    ...
    if (!current || !current.memfsEnabled)
        return;
    try {
 ...
 */
export function getDaemonPortPath() {
...
function appendInitEvent(taskId, event) {
...
function getInFlightInitTaskId(teammateName) {
...
export async function recoverStaleInitTasks() {
...
export async function startBackgroundInit(teammate, options = {
...
async function processInitTask(taskId, teammateName, prompt) {
    ...
    switch (msg.type) {
        ...
        case 'fork': {
            ...
            try {
                ...
                const target = teammate.targets?.find((entry) => entry.name === targetName);
                if (!target) {
...
export async function processTask(taskId, targetName, message, options = {}) {
    const startedAt = new Date().toISOString();
    // Update task status
    updateTask(taskId, { status: "running", startedAt });
    runningTasks.set(taskId, { startedAt });
    ...
    try {
        ...
        const completionPayload = {
            ...
            toolCalls,
            conversationId: getConversationTarget(parseTargetName(targetName).rootName, parseTargetName(targetName).fullName)?.conversationId,
        };
...
async function triggerReviewIfReady(pipelineId, options) {
...
export async function startDaemon(port = DEFAULT_PORT) {
    ...
    try {
        ...
        console.error("Error:", error instanceof Error ? error.message : "No API key found");
        process.exit(1);
    }
    // Save PID and port
    saveDaemonPid();
