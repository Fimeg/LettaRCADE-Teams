 ...
 * Set error details
 */
export function setError(name, errorDetails) {
 ...
 * Add a TODO item
 */
export function addTodo(name, input) {
 ...
 * List TODO items
 */
export function listTodoItems(name) {
    const state = loadTeammate(name);
 ...
 */
export function startTodo(name, todoId, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
...
export function blockTodo(name, todoId, reason, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
    const todoItems = [...(state.todoItems || [])];
...
export function unblockTodo(name, todoId, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
    const todoItems = [...(state.todoItems || [])];
...
export function completeTodo(name, todoId, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
    const todoItems = [...(state.todoItems || [])];
...
export function dropTodo(name, todoId, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
    const todoItems = [...(state.todoItems || [])];
...
export function updateStatusSummary(name, input, options) {
    const state = loadTeammate(name);
    if (!state)
        return null;
    const now = new Date().toISOString();
...
export function getRecentStatusEvents(name, limit = 20) {
    const state = loadTeammate(name);
    if (!state?.statusEvents)
        return [];
...
export function findStaleTeammates(maxSilentMinutes) {
...
function mapPhaseToTeammateStatus(phase) {
    if (phase === 'blocked')
        return 'error';
    if (phase === 'done')
        return 'done';
...
function generateStatusEventId() {
    const timestamp = Date.now().toString(36);
    const random = Math.random().toString(36).slice(2, 8);
    return `evt-${timestamp}-${random}`;
}
function findTodoIndex(todoItems, todoId) {
    return todoItems.findIndex((item) => item.id === todoId);
}
function requireTodo(name, todoItems, todoId) {
