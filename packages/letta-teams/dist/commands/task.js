...
`)
        ...
        if (target) {
            ...
            if (task) {
                ...
                handleError(new Error(`'${target}' is neither a known task ID nor a known teammate/target`), jsonMode);
                return;
            }
        }
        let stopped = false;
                ...
                if (jsonMode) {
                    ...
                    if (activeTasks.length === 0) {
                        ...
                        for (const task of activeTasks) {
                            ...
                            console.log(`${statusIcon} ${targetLabel.padEnd(20)} ${task.id.padEnd(24)} ${task.status.padEnd(8)} ${formatElapsed(task)}`);
                        }
                    }
...
function matchesWatchTarget(task, targetName) {
    return (task.teammateName === targetName ||
