...
`)
        ...
        try {
            ...
            const targetNames = options.to
                ? options.to.split(",").map((s) => s.trim()).filter(Boolean)
                : undefined;
            const prompt = promptParts.join(" ");
            if (!prompt) {
...
`)
        .action(async (assignments, options) => {
        const globalOpts = program.opts();
        try {
            ...
            let reviewTarget;
            let gatePolicy = options.gate === "always" ? "always" : "on_success";
            const reviewTemplate = options.reviewTemplate;
            if (options.reviewBy) {
            ...
            for (const [name, message] of messages) {
                ...
                const { taskId } = await dispatchTask(name, message, {
                    pipelineId,
                    review: reviewTarget && pipelineId
                        ? {
