...
const SKILL_SCOPES = ['project', 'agent', 'global'];
function parseScope(value) {
    if (!value)
        return 'project';
    if (SKILL_SCOPES.includes(value)) {
        return value;
    }
...
function parseLettabotWorkingDir(configContent) {
...
function resolveScopeDir(scope) {
    switch (scope) {
        case 'project':
...
export function resolveSkillSource(skillName) {
    ...
    throw new Error(`Skill '${skillName}' not found. Checked: ${candidatePaths.join(', ')}. Built-in skills: ${builtInList}`);
}
