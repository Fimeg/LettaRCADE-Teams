...
import { startDaemonInBackground, waitForDaemon, getDaemonLogPath, isDaemonRunning, } from "../ipc.js";
    ...
    program
        ...
        if (options.start || (!options.stop && !options.status)) {
            ...
            if (ready) {
                ...
                handleError(new Error(`Daemon failed to start. Check log file: ${logPath}`), globalOpts.json);
            }
        }
    });
}
