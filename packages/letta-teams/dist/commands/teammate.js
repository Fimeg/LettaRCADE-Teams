...
import { getConversationTarget, listConversationTargets, teammateExists, removeTeammate, listTeammates, loadTeammate, updateTeammate, getApiKey, findTasksToPrune, findIdleTeammates, findBrokenTeammates, deleteTasks, deleteTeammates, } from "../store.js";
import { parseTargetName, validateTargetName } from "../targets.js";
const require = createRequire(import.meta.url);
export function registerTeammateCommands(program) {
    // PRUNE COMMAND
    program
        .command("prune")
    ...
    program
        ...
        if (globalOpts.json) {
            ...
            for (const t of teammates) {
                ...
                if (t.statusSummary?.message)
                    console.log(`    Status: ${t.statusSummary.message}`);
...
`)
        ...
        if (globalOpts.json) {
            ...
            if (target.parentTargetName)
                console.log(`  Parent target: ${target.parentTargetName}`);
    ...
    program
        ...
        if (options.deleteAgent && state?.agentId) {
            ...
            if (!options.yes && !globalOpts.json) {
                ...
                const answer = await rl.question(`This will permanently delete the Letta agent '${name}' (ID: ${state.agentId}) from the server. Continue? [y/N] `);
                rl.close();
                if (answer.toLowerCase() !== "y" && answer.toLowerCase() !== "yes") {
                    console.log("Cancelled.");
                    return;
